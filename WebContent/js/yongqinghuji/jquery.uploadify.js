					...
					.each(function() {
						...
						if (args[0]) {
							...
							$item.delay(1000).fadeOut(500, function() {
								...
								$("#uploadify-queue p").remove(); //点击‘取消上传’清除div块
								$(this).remove();
	...
	var handlers = {
		...
		onSelect : function(file) {
			...
			if (settings.itemTemplate == false) {
				settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\<span class="fileName">${fileName} (${fileSize})</span><span class="data"></span>\
					<div class="uploadify-progress">\
						<div class="uploadify-progress-bar"><!--Progress Bar--></div>\
					</div>\
				</div>\<div id="uploadify-queue" class="uploadify-queue">\<p>\<a class="shangchuan" href="javascript:$(\'#uploadify\').uploadify(\'upload\')">上传</a>\<a href="javascript:$(\'#uploadify\').uploadify(\'cancel\')">取消上传</a>\</p>\</div>';
				$(".uploadify-queue").css("z-index","999"); //点击之后关闭按钮
				var left=$(".uploadify-queue").offset().left;
		 		var top=$(".uploadify-queue").offset().top;
		 		$(".uploadify-queue-item").css({"left":left,"top":top});
				$(".zhe").show();
			}
			// Run the default event handler
						...
						setTimeout(function() {
							if ($('#' + file.id)) {
								...
								$('#' + file.id).fadeOut(500, function() {
									$("#uploadify-queue p").remove(); //点击‘上传’清除div块
									$(this).remove();
									alert("上传成功");
									location.reload();
								});
